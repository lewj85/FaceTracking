...
int main( int argc, const char** argv )
{
    CommandLineParser parser(argc, argv,
        "{help h||}"
        "{face_cascade|..\source\data\haarcascades\haarcascade_frontalface_alt.xml|}"
        "{eyes_cascade|..\source\data\haarcascades\haarcascade_eye_tree_eyeglasses.xml|}");
...
void detectAndDisplay( Mat frame )
{
    ...
    for ( size_t i = 0; i < faces.size(); i++ )
    {
		...
		for ( size_t k = 0; k < eyes.size()/2; ++k )
        {
			...
			//int radius = 1;
			//circle( frame, between_eyes, radius, Scalar( 255, 0, 0 ), 4, 8, 0 );
			Point forehead( faces[i].x + (left_eye_x + right_eye_x)/2, faces[i].y + 0.75*(left_eye_y + right_eye_y)/2 );
			int radius = 1
			circle( frame, between_eyes, radius, Scalar( 255, 0, 0 ), 4, 8, 0 );
		}
    }
